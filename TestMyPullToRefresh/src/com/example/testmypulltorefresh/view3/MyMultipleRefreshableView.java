...
public class MyMultipleRefreshableView extends LinearLayout implements OnTouchListener{
	...
	private void findTargetView(ViewGroup viewGroup) {
		for (int i = 0; i < viewGroup.getChildCount(); i++) {
			if(isFindTargetView) return;
			if (viewGroup.getChildAt(i) instanceof AdapterView
							|| viewGroup.getChildAt(i) instanceof ScrollView 
							|| viewGroup.getChildAt(i) instanceof HorizontalScrollView) {
	...
	private boolean touch(MotionEvent event, boolean isInterrupt){
		setIsAbleToPull(mTargetView, event);
		if (mAbleToPull) {
			...
			if (mCurrentStatus == STATUS_PULL_TO_REFRESH
				...
				if (mTargetView != null) {
					...
					mTargetView.setFocusable(false);
					mTargetView.setFocusableInTouchMode(false);
				}
	 ...
	 * @param event
	 */
	private void setIsAbleToPull(ViewGroup targetView, MotionEvent event) {
		if (targetView == null) {
			if (!mAbleToPull) {
		...
		if (targetView instanceof AdapterView) {
			View firstChild = targetView.getChildAt(0);
			if (firstChild != null) {
				int firstVisiblePos = ((AdapterView) targetView)
						.getFirstVisiblePosition();
				if (DEBUG)
					Log.v(TAG, "firstChild getTop: " + firstChild.getTop());
		...
		} else if (targetView instanceof HorizontalScrollView) {
			mAbleToPull = true;
		} else {
			mAbleToPull = true;
		}
